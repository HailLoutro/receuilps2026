...
import { Users, Pencil, Lock, Download, Archive, Layers, LogOut, CheckCircle2, Clock } from "lucide-react";
    ...
    <div className="flex h-screen overflow-hidden" style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", background: "#f1f5f9" }}>
      ...
      <aside className="w-60 bg-[#0a0e3a] text-white flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 p-3 space-y-1">
          {NAV.map(n => (
            <button key={n.id} onClick={() => setView(n.id)} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all ${view === n.id ? "bg-white/15 font-semibold text-white" : "text-blue-200/70 hover:bg-white/5 hover:text-white"}`}>
